    ...
    <div className="flex flex-col min-h-screen bg-[var(--color-surface)] text-[var(--color-text-primary)]">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-8 space-y-8 animate-fade-in">
        ...
        <div className="space-y-2 border-b border-[var(--color-border)] pb-6">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[var(--color-primary-light)] text-[var(--color-primary)] border border-blue-200 dark:border-blue-900/30">
        ...
        <Tabs defaultValue="tokens" className="w-full space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {COLOR_TOKENS.map((token) => (
                <button
                  ...
                  className="flex flex-col p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)] text-left cursor-pointer transition-colors shadow-xs group"
                >
                  <div
                    className="w-full h-24 rounded-lg border border-[var(--color-border)] shadow-inner mb-3 transition-transform group-hover:scale-[1.01]"
                    style={{ backgroundColor: `var(${token.variable})` }}
                  />
                  <span className="text-xs font-bold text-[var(--color-text-primary)]">{token.name}</span>
                  <span className="font-mono text-[10px] text-[var(--color-primary)] font-semibold mt-0.5">
                    {token.variable}
                  </span>
          ...
          <TabsContent value="typography" className="space-y-6 animate-fade-in">
            ...
            <div className="p-6 border border-[var(--color-border)] rounded-xl bg-[var(--color-surface-alt)] space-y-6">
              <div className="space-y-1 pb-4 border-b border-[var(--color-border)]">
                <span className="text-[10px] font-mono text-[var(--color-primary)] font-bold">h1. Heading Large (30px / 1.875rem)</span>
                <h1 className="text-3xl font-extrabold tracking-tight">The quick brown fox jumps over the lazy dog</h1>
              </div>

              <div className="space-y-1 pb-4 border-b border-[var(--color-border)]">
                <span className="text-[10px] font-mono text-[var(--color-primary)] font-bold">h2. Heading Medium (20px / 1.25rem)</span>
                <h2 className="text-lg font-bold">The quick brown fox jumps over the lazy dog</h2>
              </div>

              <div className="space-y-1 pb-4 border-b border-[var(--color-border)]">
                <span className="text-[10px] font-mono text-[var(--color-primary)] font-bold">h3. Subtitle / Label (14px / 0.875rem)</span>
                <h3 className="text-sm font-semibold">The quick brown fox jumps over the lazy dog</h3>
              </div>

              <div className="space-y-1 pb-4 border-b border-[var(--color-border)]">
                <span className="text-[10px] font-mono text-[var(--color-primary)] font-bold">p. Body Regular (14px / 0.875rem / Leading Relaxed)</span>
              ...
              <div className="space-y-1">
                <span className="text-[10px] font-mono text-[var(--color-primary)] font-bold">code. Monospace Code (13px / 0.8125rem)</span>
                <pre className="font-mono text-xs text-[var(--color-text-primary)] bg-[var(--color-surface)] p-3 rounded-lg border border-[var(--color-border)]">
                  const app = &quot;Toolify&quot;;
                </pre>
              </div>
            </div>
          ...
          <TabsContent value="components" className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 border-b border-[var(--color-border)] pb-8">
              <div className="lg:col-span-5 space-y-4">
                <Label className="text-xs font-bold text-[var(--color-text-secondary)] uppercase tracking-wider">
                  1. Live DropZone Demo
                </Label>
                <div className="p-4 bg-[var(--color-surface-alt)] border border-[var(--color-border)] rounded-xl min-h-[160px] flex items-center justify-center">
                  <DropZone
                ...
                {simulatedFiles.length > 0 && (
                  <p className="text-xs text-[var(--color-success)] font-semibold text-center">
                    Files loaded: {simulatedFiles.map((f) => f.name).join(', ')}
                  </p>
                )}
              </div>
              <div className="lg:col-span-7 space-y-2">
                <Label className="text-xs font-semibold text-[var(--color-text-secondary)]">TSX Usage Code</Label>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 border-b border-[var(--color-border)] pb-8">
              <div className="lg:col-span-5 space-y-4">
                ...
                <div className="p-4 bg-[var(--color-surface-alt)] border border-[var(--color-border)] rounded-xl flex items-center justify-center min-h-[100px]">
                  <ProcessingButton
                ...
                <div className="flex justify-between items-center">
                  ...
                  {outputBlob && (
                    <Button
                      ...
                      className="text-[10px] text-[var(--color-danger)] h-7 cursor-pointer"
                    >
                      Clear State
                    </Button>
                  )}
                </div>
                <div className="p-4 bg-[var(--color-surface-alt)] border border-[var(--color-border)] rounded-xl min-h-[120px] flex items-center justify-center">
                  {outputBlob ? (
                    <OutputArea
                      mode="download"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-5 space-y-4">
                ...
                <div className="p-4 bg-[var(--color-surface-alt)] border border-[var(--color-border)] rounded-xl min-h-[140px] flex items-center justify-center">
