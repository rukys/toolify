      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full p-4 rounded-xl border border-(--color-border) bg-(--color-surface-alt) font-mono text-xs focus:border-(--color-primary) focus:outline-none placeholder:text-(--color-text-muted) break-all"
            />
          </div>
          ...
          {!decoded && !error && (
            <div className="p-4 rounded-xl border border-(--color-border) bg-(--color-surface-alt) text-xs text-(--color-text-muted) flex items-start gap-2">
              <Info className="w-4 h-4 shrink-0 mt-0.5 text-(--color-primary)" />
        ...
        <div className="lg:col-span-7 space-y-6">
          {decoded ? (
            <>
              ...
              {claimsList.length > 0 && (
                <div className="rounded-xl border border-(--color-border) bg-(--color-surface-alt) p-4 space-y-3">
                  <span className="text-xs font-bold text-(--color-text-primary)">Parsed Claims Info</span>
                  <div className="space-y-2 text-xs">
                    {claimsList.map((claim) => (
                      <div key={claim.name} className="grid grid-cols-1 sm:grid-cols-3 gap-1 border-b border-(--color-border)/50 pb-2 last:border-b-0 last:pb-0">
                        <span className="font-semibold text-(--color-primary)">{claim.name}</span>
                        <span className="sm:col-span-2 font-mono text-(--color-text-primary) break-all">{claim.value}</span>
                        <span className="sm:col-start-2 sm:col-span-2 text-(--color-text-muted) text-[10px]">{claim.desc}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-4 rounded-xl border border-(--color-border) bg-(--color-surface-alt) space-y-4">
                  <div className="flex items-start gap-3 text-xs text-(--color-text-secondary)">
                    <ShieldAlert className="w-5 h-5 shrink-0 text-amber-500 mt-0.5" />
                    <div>
                      <p className="font-semibold text-(--color-text-primary)">Signature Verification Disclaimer</p>
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] text-(--color-text-muted) uppercase font-semibold">Raw Signature</span>
                    <pre className="p-3 text-xs font-mono rounded-lg bg-(--color-surface) border border-(--color-border) overflow-x-auto break-all whitespace-pre-wrap select-all">
                      {decoded.signature}
                    </pre>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-(--color-text-muted)">
                      Algorithm: <strong className="font-semibold text-(--color-text-primary)">{decoded.header.alg || 'unknown'}</strong>
                    </span>
            ...
            <div className="flex flex-col items-center justify-center h-48 border border-dashed border-(--color-border) rounded-2xl bg-(--color-surface-alt) text-(--color-text-muted)">
